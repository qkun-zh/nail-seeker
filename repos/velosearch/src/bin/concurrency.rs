...
async fn main_inner(index_dir: String, partitions_num: usize) -> Result<()> {
    let posting_table = Arc::new(deserialize_posting_table(index_dir, partitions_num).unwrap());

    let ctx = BooleanContext::new();
    ctx.register_index(TableReference::Bare { table: "__table__".into() }, posting_table.clone())?;
    ...
    for line_res in stdin.lines() {
        ...
        if hint != concurrency {
            posting_table.as_ref().partitions_num.swap(hint, Ordering::Acquire);
            concurrency = hint;
        }
